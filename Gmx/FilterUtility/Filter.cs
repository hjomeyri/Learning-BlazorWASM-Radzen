...
namespace Gmx.FilterUtility
{
    public class Filter<T> : object
    {
        ...
        private static IEnumerable<T> FilterData(FilterOptions filterOption, IEnumerable<T> data,
            ...
            switch (filterOption)
            {
                ...
                case FilterOptions.StartsWith:
                    data = data.Where(x => filterColumn.GetValue(x, null) != null && filterColumn.GetValue(x, null)
                        .ToString().ToLower().StartsWith(filterValue.ToLower())).ToList();
                    break;
                case FilterOptions.EndsWith:
                    data = data.Where(x => filterColumn.GetValue(x, null) != null && filterColumn.GetValue(x, null)
                        .ToString().ToLower().EndsWith(filterValue.ToLower())).ToList();
                    break;
                case FilterOptions.Contains:
                    data = data.Where(x => filterColumn.GetValue(x, null) != null && filterColumn.GetValue(x, null)
                        .ToString().ToLower().Contains(filterValue.ToLower())).ToList();
                    break;
                ...
                case FilterOptions.IsEqualTo:
                    ...
                    else
                    {
                        if ((filterColumn.PropertyType == typeof(int) ||
                            ...
                            data = data.Where(x =>
                                filterColumn.GetValue(x, null) != null &&
                                filterColumn.GetValue(x, null).ToString().ToLower() == filterValue.ToLower()).ToList();
                    }
